...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "fk_refresh_tokens_user_id_users", "refresh_tokens", schema="security", type_="foreignkey"
    )
    op.drop_constraint(
        "fk_audit_log_user_id_users", "audit_log", schema="security", type_="foreignkey"
    )
    op.drop_constraint(
    ...
    op.drop_constraint(
        "fk_notifications_user_id_users", "notifications", schema="core", type_="foreignkey"
    )
    op.drop_constraint(
        "fk_notifications_tenant_id_tenants", "notifications", schema="core", type_="foreignkey"
    )
    ...
    op.execute(
        sa.text("ALTER TABLE security.users ALTER COLUMN tenant_id TYPE uuid USING tenant_id::uuid")
    )
    op.execute(
        sa.text("ALTER TABLE security.refresh_tokens ALTER COLUMN id TYPE uuid USING id::uuid")
    )
    op.execute(
        sa.text(
            "ALTER TABLE security.refresh_tokens ALTER COLUMN user_id TYPE uuid USING user_id::uuid"
        )
    )
    op.execute(sa.text("ALTER TABLE security.audit_log ALTER COLUMN id TYPE uuid USING id::uuid"))
    op.execute(
        sa.text("ALTER TABLE security.audit_log ALTER COLUMN user_id TYPE uuid USING user_id::uuid")
    )
    op.execute(
        sa.text("ALTER TABLE security.login_protection ALTER COLUMN id TYPE uuid USING id::uuid")
    )
    op.execute(
        sa.text(
            "ALTER TABLE core.tenant_company_settings "
            "ALTER COLUMN tenant_id TYPE uuid USING tenant_id::uuid"
        )
    )
    op.execute(
        sa.text(
            "ALTER TABLE core.tenant_smtp_settings "
            "ALTER COLUMN tenant_id TYPE uuid USING tenant_id::uuid"
        )
    )
    op.execute(
        sa.text(
            "ALTER TABLE core.tenant_document_sequences ALTER COLUMN id TYPE uuid USING id::uuid"
        )
    )
    op.execute(
        sa.text(
            "ALTER TABLE core.tenant_document_sequences "
            "ALTER COLUMN tenant_id TYPE uuid USING tenant_id::uuid"
        )
    )
    op.execute(sa.text("ALTER TABLE core.boms ALTER COLUMN id TYPE uuid USING id::uuid"))
    op.execute(
        sa.text("ALTER TABLE core.boms ALTER COLUMN tenant_id TYPE uuid USING tenant_id::uuid")
    )
    op.execute(
        sa.text("ALTER TABLE core.finance_cost_entries ALTER COLUMN id TYPE uuid USING id::uuid")
    )
    op.execute(
        sa.text(
            "ALTER TABLE core.finance_cost_entries "
            "ALTER COLUMN tenant_id TYPE uuid USING tenant_id::uuid"
        )
    )
    op.execute(
    ...
    op.execute(
        sa.text("ALTER TABLE core.notifications ALTER COLUMN user_id TYPE uuid USING user_id::uuid")
    )
    op.execute(
        sa.text(
            "ALTER TABLE core.notifications ALTER COLUMN tenant_id TYPE uuid USING tenant_id::uuid"
        )
    )
...
def downgrade() -> None:
    ...
    op.execute(
        sa.text("ALTER TABLE security.tenants ALTER COLUMN id TYPE varchar(36) USING id::text")
    )
    op.execute(
        sa.text("ALTER TABLE security.users ALTER COLUMN id TYPE varchar(36) USING id::text")
    )
    op.execute(
        sa.text(
    ...
    op.execute(
        sa.text(
            "ALTER TABLE security.refresh_tokens ALTER COLUMN id TYPE varchar(36) USING id::text"
        )
    )
    op.execute(
        sa.text(
    ...
    op.execute(
        sa.text("ALTER TABLE security.audit_log ALTER COLUMN id TYPE varchar(36) USING id::text")
    )
    op.execute(
        sa.text(
    ...
    op.execute(
        sa.text(
            "ALTER TABLE security.login_protection ALTER COLUMN id TYPE varchar(36) USING id::text"
        )
    )
    op.execute(
        sa.text(
    ...
    op.execute(
        sa.text(
            "ALTER TABLE core.boms ALTER COLUMN tenant_id TYPE varchar(36) USING tenant_id::text"
        )
    )
    op.execute(
        sa.text(
            "ALTER TABLE core.finance_cost_entries ALTER COLUMN id TYPE varchar(36) USING id::text"
        )
    )
    op.execute(
        sa.text(
    ...
    op.execute(
        sa.text("ALTER TABLE core.notifications ALTER COLUMN id TYPE varchar(36) USING id::text")
    )
    op.execute(
        sa.text(
